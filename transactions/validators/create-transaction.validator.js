...
export function validadeTransaction(request, response, next) {
    const date = request.body.date;
    if (!date) {
        return response.status(400).json(new BadRequestError("Data nao informada"));
    }
    const isTimestamp = date._seconds != null || date.seconds != null;
    if (!isTimestamp && isNaN(new Date(date))) {
    ...
    if (!money.currency) {
        return response.status(400).json(new BadRequestError("Moeda nao informado"));
    }
    if (!money.value) {
